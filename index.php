...
<body>
      ...
      <div id="templatemo_slider"><span class="sliderframe"></span>
        <div class="slider-wrapper theme-orman">
          <div id="slider" class="nivoSlider" style="position: relative; background: url(&quot;images/slider/04.jpg&quot;) no-repeat;">
            ...
            ?>
              <img width="50" src="admin/slider/gambar/<?= $data['gambar'] ?>" alt="image 6" />
            <?php } ?>
          </div>
        </div>
  ...
  <div id="templatemo_main"><span class="mf mft"></span><span class="mf mfb"></span> <?php include "koneksi.php";
                                                                                      $query = mysqli_query($koneksi, "SELECT * FROM data_barang");
                                                                                      while ($data = mysqli_fetch_array($query)) { ?> <div class="col col_3">
        <h2><?= $data['nama_barang'] ?></h2> <img width="200" src="admin/data_barang/gambar/<?= $data['gambar'] ?>" alt="image 1" class="img_nom img_border img_border_b" />
        <p><em><?= $data['deskripsi'] ?></em>
        </p>
        <ul class="list_bullet">
          <li>Rp. <?= number_format($data['harga']) ?></li>
        </ul> <a href="#" class="more">Beli</a>
      </div> <?php } ?> <div class="clear"></div>
  </div> <!-- END of main -->
  <?php
  include "footer.php";
  ?>
  <div id="templatemo_footer"> Copyright © 2023 E-Katalog </div> <!-- END of footer -->
  </div> <!-- END of wrapper -->
</body>
